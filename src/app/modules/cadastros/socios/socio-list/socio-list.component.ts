...
export class AeronaveSocioListComponent implements OnInit, OnDestroy {

    @ViewChild(MatPaginator) private _paginator: MatPaginator;
    @ViewChild(MatSort) private _sort: MatSort;

    socios: AeronaveSocio[] = [];
    aeronaveId: string;
    ...
    productsTableColumns: string[] = ['nome', 'percentual', 'actions'];
    ...
    load() {
        this.isLoading = true;
        this._aeronaveSocioService.listById(this.query, this.aeronaveId).subscribe(result => {
    ...
    details(id: string) {
        ...
        this.router.navigate(['/socios/details/' + this.selectedId]);
    }

    edit(id: string) {
        this.selectedId = id;

        this.router.navigate(['/socios/atualizar/' + this.selectedId]);
    }

    delete(id: string) {
        this.isLoading = true;
        this._aeronaveSocioService.delete(id).subscribe(_ => {
    ...
    deletar(id: string) {
        ...
        const confirmation = this._fuseConfirmationService.open({
            title: 'Deletar Sócio',
            message:
                'Tem certeza que deseja deletar o sócio? Não será possível desfazer.',
            actions: {
                confirm: {
                    label: 'Deletar',
                },
        ...
        confirmation.afterClosed().subscribe((result) => {
            ...
            if (result === 'confirmed') {
                const $obs = this._aeronaveSocioService.updateDeleting(id);
    ...
    novo(){
         this.router.navigate(['/socios/' + this.aeronaveId + '/novo']);
    }
}
